...
_C.TREE_MODEL = CN()
# random state for tree based model
_C.TREE_MODEL.RANDOM_STATE=[np.random.RandomState(_C.MODEL.RANDOM_STATE)]
# list of params_grid
...
# default infered well performed model
_C.TOP_MODELS = CN()
_C.TOP_MODELS.MODEL = ["random forest (tuned)", "xgboost (tuned)", "lgbm (tuned)"]

# select model for interpretation
_C.MODEL_INTERPRET = CN()
_C.MODEL_INTERPRET.NAME = "lgbm (tuned)"
